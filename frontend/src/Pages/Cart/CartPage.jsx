...
export default function CartPage({ history }) {
  ...
  return (
    <Cart className="px-5 md:px-20 lg:px-24 flex font-sans flex-col bg-gray-50  py-5">
      <section>
      ...
      <h1 className="pl-2 text-center lg:align-baseline border-b mb-2 py-2 lg:py-5 text-lg md:text-xl font-bold text-secondary">My Cart</h1>
      ...
      <CartFlex className="flex flex-col lg:flex-row lg:space-x-3 font-sans space-y-3 lg:space-y-0 ">
        ...
        <OrderSummaryRight
          className={`${cartItems.length > 2 && 'hidden'} lg:block lg:w-96 font-sans bg-white p-3 lg:p-5 rounded shadow-lg max-h-80`}
        >
          ...
          <div className="space-y-2">
            ...
            <button
              ...
              className="bg-yellow-500  space-x-1 text-sm leading-5 focus:outline-none py-2 flex justify-center items-center w-full"
            >
              <ShoppingCartIcon />
      ...
      <OrderSummaryButtom
        className={`${cartItems.length <= 2 && 'hidden'} lg:hidden fixed bottom-14 block h-14 w-full left-0 bg-gray-100 shadow-lg `}
      >
        <div className="flex justify-end items-center px-10 h-5/6 ">
          ...
          <button onClick={checkoutHandler} className="bg-yellow-500  px-5 py-3 mt-2">
            Checkout
          </button>
        </div>
      </OrderSummaryButtom>
